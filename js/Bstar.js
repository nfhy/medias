...
$(function(){
    twiceSize = true;
    initGrids();
    $("#restart_BST").bind("click",function(){
        if(timer_bst){clearInterval(timer_bst);}
        BST.prepareToRunWhenResetEndPoint(14,9);
    });
    allGrids().bind("click",function(){
        if(timer_bst){
            var x = $(this).attr("x");
            var y = $(this).attr("y");
            BST.prepareToRunWhenResetEndPoint(x,y);
        }
    });
});
var BST = {
    init : function () {
        ...
        allGrids().each(function () {
            ...
            var y = grid.attr("y");
            var key = x + "," + y;
            gridDataDic[key] = BST.GridData(x, y);
        });
        BST.setStart(0, 0);
        BST.setImpassable({"x": 5, "y": 0}, {"x": 6, "y": 8});
        BST.setImpassable({"x": 1, "y": 8}, {"x": 6, "y": 8});
        //BST.setImpassable({"x": 25, "y": 7},{"x": 26, "y": 14});
        //BST.setImpassable({"x": 1, "y": 13}, {"x": 21, "y": 13});
        //BST.render();
        //timer_bst = setInterval("BST.stepForward();", 1);
    },
            ...
            for (var nearByGridKeyIndex in nearByGridKeys) {
                ...
                if (gridDataDic[nearByGridKey]) {
                    ...
                    if (!openList.contains(nearByGridKey)) {
                        ...
                        nearByGrid.costAll = BST.heuristic(nearByGridKey)+nearByGrid.costNow;
                        ...
                        if(newCostNow<nearByGrid.costNow){
                            ...
                            nearByGrid.costAll = newCostNow + BST.heuristic(nearByGridKey);
                            nearByGrid.parent = key;
                            changeList.pushIfNotExists(nearByGridKey);
                        }
                    }
    ...
    heuristic : function(key){
        ...
        if(diagonalMove){
            ...
            return (Math.abs(x-endx)+Math.abs(y-endy))*distance;
    },
    /**
     *地图区块数据结构
     */
